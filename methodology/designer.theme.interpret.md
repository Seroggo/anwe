...
### Правило достаточности (blocking vs non-blocking)

Интерпретация блокируется только тогда, когда отсутствует **grounded business context**
целиком: нет `Business_type` и нет иных подтверждённых business-признаков, из которых
визуальное направление можно вывести.

Пустой или отсутствующий `Desired_character[]` **не является** блокирующим условием.
Если SiteContext/SiteModel дают достаточный business context — тип бизнеса, аудитории,
позиционирование, offers, constraints, — Interpreter обязан:

1. самостоятельно выбрать согласованное визуальное направление;
2. зафиксировать этот выбор как `INTERPRETER_DECISION` или `FALLBACK`
   в `decisions[]` с `evidence` и `rationale`;
3. не выдумывать business facts;
4. не перекладывать дизайнерское решение на пользователя.

`DATA_GAP` в этом случае фиксируется только по конкретному неподтверждённому свойству
(например, реальные brand colors), но не по факту отсутствия `Desired_character`.
